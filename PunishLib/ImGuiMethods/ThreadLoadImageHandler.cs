...
using static PunishLib.GenericHelpers;
                    ...
                    Safe(delegate
                    {
                        if (CachedTextures.TryGetFirst(x => x.Value.isCompleted == false, out var keyValuePair))
                        {
                            ...
                            if (keyValuePair.Key.StartsWith("http:", StringComparison.OrdinalIgnoreCase) || keyValuePair.Key.StartsWith("https:", StringComparison.OrdinalIgnoreCase))
                            {
                                ...
                                keyValuePair.Value.texture = PunishLibMain.PluginInterface.UiBuilder.LoadImage(content);
                            }
                            else
                            {
                                keyValuePair.Value.texture = PunishLibMain.PluginInterface.UiBuilder.LoadImage(keyValuePair.Key);
                            }
                        }
                    });
                    idleTicks++;
